...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    private static final String TAG = "@vir ImageViewTouchBase";
     ...
     */
    public void setImageDrawable(final Drawable drawable, final Matrix initial_matrix, final
    float min_zoom, final float max_zoom) {
    ...
    protected void _setImageDrawable(final Drawable drawable, final Matrix initial_matrix, float
            min_zoom, float max_zoom) {
        ...
        if (min_zoom != ZOOM_INVALID && max_zoom != ZOOM_INVALID) {
            ...
            if (mScaleType == DisplayType.FIT_TO_SCREEN || mScaleType == DisplayType
                    .FIT_IF_BIGGER) {
    ...
    public void saveMatrixInfo(Matrix matrix) {
        mScaleX = getValue(matrix, Matrix.MSCALE_X);
        mScaleY = getValue(matrix, Matrix.MSCALE_Y);
        mTransX = getValue(matrix, Matrix.MTRANS_X);
        mTransY = getValue(matrix, Matrix.MTRANS_Y);
        Log.d(LOG_TAG, "matrix: { x: " + mTransX + ", y: " + mTransY + ", scalex: " + mScaleX +
                ", " + "scaley: " + mScaleY + " }");
    }
